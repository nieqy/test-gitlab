...
@Component
public class VideoDownloadTask implements SchedulingConfigurer{
	...
	private static final String REDIS_QUEUE_KEY = "video_download_redis_queue_key_";

	private static final String REDIS_SET_KEY = "video_download_redis_set_key_";
	...
	private static String taskLable = "video";
			...
			while (allNetInterfaces.hasMoreElements()) {
				...
				while (addresses.hasMoreElements()) {
					...
					if (ip != null && ip instanceof java.net.Inet4Address) {
						...
						REDIS_POST_SET_KEY = "video_download_redis_post_set_key_" + localIP;
					}
	...
	public void deleteOldData() {
		...
		if (this.jedisService.setIfAbsent("video_download_redis_set_key_exist_deleteOldData", Integer.valueOf(1), 30L, TimeUnit.SECONDS)) {
			List<MediaInfoEntity> deleteMedias = this.videoInfoMapper.selectDeleteMedia();
			for (MediaInfoEntity media : deleteMedias) {
				if (media.getCreateTime().before(TimeUtils.getDateIncDayCount(new Date(), -5))) {
					this.videoInfoMapper.deleteMedia(media.getMaterialId());
					logger.info("delete data media:id:{},name:{},size:{},path:{}", new Object[] { media.getMaterialId(), media.getFileName(), media.getFileSize(), media.getFilePath() });
				}
			}
		}
	...
	public void deletePostFile() {
		...
		List<MediaInfoEntity> medias = this.videoInfoMapper.selectFinishedMedia(localIP);
		for (MediaInfoEntity media : medias) {
			File file = new File(media.getvFilePath());
			if (file.exists()) {
				file.delete();
			}
			this.videoInfoMapper.deleteFinishedMedia(media.getMaterialId());
			logger.info("delete media:id:{},name:{},size:{},path:{}", new Object[] { media.getMaterialId(), media.getFileName(), media.getFileSize(), media.getvFilePath() });
		}
	}
	...
	public void putDataInRedisQueue() {
		...
		logger.info("*********************video download task run:" + localIP + "*****************************");
		if (this.jedisService.setIfAbsent("video_download_redis_set_key_exist", Integer.valueOf(1), 30L, TimeUnit.SECONDS)) {
			Set<MediaInfoEntity> medias = this.videoInfoMapper.selectDownloadMedia();
			medias = (medias == null) ? new HashSet<>() : medias;
			Set<MediaInfoEntity> existsMedias = (Set<MediaInfoEntity>)this.jedisService.get("video_download_redis_set_key_");
			...
			for (MediaInfoEntity media : medias) {
				if (!existsMedias.contains(media) && media.getCreateTime().after(TimeUtils.getDateIncDayCount(new Date(), -2))) {
					this.jedisService.leftPush("video_download_redis_queue_key_", media);
				}
			}
			existsMedias.addAll(medias);
			this.jedisService.set("video_download_redis_set_key_", existsMedias);
		}
	}
	...
	public void postTags() {
		...
		logger.info("*********************post data to linda:" + localIP + "*****************************");
		List<MediaInfoEntity> medias = this.videoInfoMapper.selectTagsMedia(localIP);
		for (MediaInfoEntity media : medias) {
			Set<MediaInfoEntity> existsMedias = (Set<MediaInfoEntity>)this.jedisService.get("video_download_redis_set_key_");
			...
			try {
				...
				params.put("process_type", "2");
				params.put("data", media.getDirtywords());
				res = HttpClientUtils.post("http://172.17.26.92:8080/linda/pcwCallBack", params);
				if (!StringUtils.isEmpty(res)) {
					JSONObject json = JSON.parseObject(res);
					if ("success".equals(json.getString("result"))) {
						this.videoInfoMapper.updateTagsMedia(media.getMaterialId());
					}
				}
	...
	public void postDownload() {
		...
		logger.info("*********************post video to service:" + localIP + "*****************************");
		List<MediaInfoEntity> media1 = this.videoInfoMapper.selectPostMedia(localIP);
		List<MediaInfoEntity> media2 = this.videoInfoMapper.selectPostedMedia(localIP);
	...
	public void downloadQueue() {
		...
		try {
			...
			while (null != (media = (MediaInfoEntity)this.jedisService.leftPop("video_download_redis_queue_key_"))) {
				this.sem.acquire();
			...
			if (null == media) {
				this.jedisService.delete("video_download_redis_set_key_");
			}
	...
	public void doDownload(MediaInfoEntity media) {
		try {
			List<MediaInfoEntity> medias = this.videoInfoMapper.selectMediaInfoById(media.getMaterialId());
			if (!this.downQueue.contains(media) && "N".equals(((MediaInfoEntity)medias.get(0)).getvStatus())) {
				this.downQueue.add(media);
				String day = TimeUtils.formatStringOnly(TimeUtils.getCurrentDate());
				String path = VideoDownloadTask.filePath + File.separator + day + File.separator;
				...
				try {
					...
					if (downFile.length() == Long.parseLong(media.getFileSize())) {
						this.videoInfoMapper.updateDownloadMedia(media.getMaterialId(), new Date(), filePath, localIP);
					}
				} catch (Exception e) {

					logger.error(e.getLocalizedMessage(), e);
				}
				this.videoInfoMapper.updateDownloadTimesMedia(media.getMaterialId());

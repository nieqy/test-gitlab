...
public class RedisService {
    ...
    private Map<RedisTemplate, Boolean> redisTemplateMap = new ConcurrentHashMap<RedisTemplate, Boolean>();

    private ConsistentHash<RedisTemplate> consistentHash;

    private static Map<RedisTemplate, Integer> redisFailCountMap = new ConcurrentHashMap<RedisTemplate, Integer>();
    ...
    public void init() {
        if (redisTemplates != null) {
            for (RedisTemplate redisTemplate : redisTemplates) {
                redisTemplateMap.put(redisTemplate, true);
            }
            consistentHash = new ConsistentHash<RedisTemplate>(redisTemplates);
            timer.schedule(new TimerTask() {
                public void run() {
                    for (RedisTemplate redisTemplate : redisTemplates) {
                        ...
                        try {
                            connectionFactory = ((LettuceConnectionFactory) redisTemplate.getConnectionFactory());
                            connection = connectionFactory.getConnection();
                            String result = connection.ping();
                            if ("PONG".equals(result)) {
                                if (!redisTemplateMap.get(redisTemplate)) {
                                    redisTemplateMap.put(redisTemplate, true);
                                    consistentHash.add(redisTemplate);
                                }
                                redisFailCountMap.remove(redisTemplate);
                            } else {
                                logger.info("Redis_Check_" + connectionFactory.getHostName() + ":" + connectionFactory.getPort() + ":" + result);
                                if (redisTemplateMap.get(redisTemplate)) {
                                    if (redisFailCountMap.get(redisTemplate) == null) {
                                        redisFailCountMap.put(redisTemplate, 1);
                                    } else {
                                        redisFailCountMap.put(redisTemplate, redisFailCountMap.get(redisTemplate) + 1);
                                    }
                                    if (redisFailCountMap.get(redisTemplate) > 3) {
                                        redisTemplateMap.put(redisTemplate, false);
                                        consistentHash.remove(redisTemplate);
                                        redisFailCountMap.remove(redisTemplate);
                                    }
                                }
                            }
                            ...
                            if (connectionFactory != null) {
                                logger.info("Redis_Check_" + connectionFactory.getHostName() + ":" + connectionFactory.getPort() + ":Error");
                            }
                            logger.error("Redis_Check_" + e.getLocalizedMessage(), e);
                            if (redisTemplateMap.get(redisTemplate)) {
                                if (redisFailCountMap.get(redisTemplate) == null) {
                                    redisFailCountMap.put(redisTemplate, 1);
                                } else {
    ...
    private RedisTemplate getRedisTemplate(String key) {
        return consistentHash.getShardInfo(key);
    }
    ...
    public void set(String key, String value) {
        final RedisTemplate redisTemplate = getRedisTemplate(key);
    ...
    /**
     * 设置 String 类型 key-value 并添加过期时间,key已经存在则返回false
     *
     * @param key
     * @param value
     * @param time  过期时间,分钟单位
    ...
    /**
     * 获取 key 和 集合  collections 中的 key 集合的差集     * @param key
     *
     * @param otherKeys
     * @return
     */

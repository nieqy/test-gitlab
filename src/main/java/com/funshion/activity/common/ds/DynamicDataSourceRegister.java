...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
        // 获取所有数据源配置
        Map defauleDataSourceProperties = binder.bind("custom.datasource.master", Map.class).get();
        Map commonProperties = binder.bind("spring.datasource", Map.class).get();
        // 获取数据源类型
        String typeStr = evn.getProperty("custom.datasource.master.type");
        // 获取数据源类型
        Class<? extends DataSource> clazz = getDataSourceType(typeStr);
        // 绑定默认数据源参数 也就是主数据源
        DataSource consumerDatasource, defaultDatasource = bind(clazz, getAllProperties(defauleDataSourceProperties, commonProperties));
        DynamicDataSourceContextHolder.dataSourceIds.add("master");
        logger.info("注册默认数据源成功");
        // 获取其他数据源配置
        String csDataSourceNames = binder.bind("custom.datasource.names", String.class).get();
        for (String name : StringUtils.split(csDataSourceNames, ",")) {
            Map paraMap = binder.bind("custom.datasource." + name, Bindable.of(Map.class)).get();
            clazz = getDataSourceType((String) paraMap.get("type"));
            // 绑定参数
            consumerDatasource = bind(clazz, getAllProperties(paraMap, commonProperties));
    ...
    private Map getAllProperties(Map defauleDataSourceProperties, Map commonProperties) {
        Map allProperties = new HashMap();
        allProperties.putAll(defauleDataSourceProperties);
        allProperties.putAll(commonProperties);
